...
const now = Date.now();
const sess = (id: number, exId: number, seconds: number) => ({
  id, exercise_id: exId, notes: null, rating: "Good", bpm: null, seconds,
  created_timestamp: now, updated_timestamp: now,
});
const child = {
  id: 2, name: "Day 21-26 Sprint", order: 1, session_type: "exercise", parent_exercise_id: 1,
  ...
  meta: { user_exercise: null, sessions: [sess(2, 2, 900)] },
};
const mockDashboard = {
  scale: null, key_signature: null, overdue: [], to_review: { songs: [] }, to_learn: { songs: [] },
  project: { songs: [] },
  exercises: [{
    id: 1, name: "30-Day Downpicking", order: 1, session_type: "exercise", parent_exercise_id: null,
    created_timestamp: 0, updated_timestamp: 0, child_exercises: [child],
    resources: null,
    meta: { user_exercise: null, sessions: [sess(1, 1, 120)] },
  }],
...
test("playing a child shows child and parent in the title and a child/parent timer", async ({ page }) => {
  await expect(page.locator("h1", { hasText: "Practice Hub" })).toBeVisible();
  await page.locator(".item-group", { hasText: "Exercises" }).locator(".item-group-header").click();
  await page.locator(".item-card", { hasText: "30-Day Downpicking" }).first().locator(".btn-collapse").click();
  const childCard = page.locator(".item-card.child-card", { hasText: "Day 21-26 Sprint" });
  ...
  const name = page.locator(".media-player__item-name");
  await expect(name).toContainText("Day 21-26 Sprint");
  await expect(name).toContainText("30-Day Downpicking");

  // Child has nothing running yet; parent = today's saved time (2m + 15m).
  await expect(page.locator(".media-player__session-timer")).toHaveText("0:00/17:00");
});
