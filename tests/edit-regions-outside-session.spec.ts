...
test("a resource's regions can be opened and edited without starting a timer or logging a session", async ({ page }) => {
  ...
  // Neither timer button was clicked — this card must not be "in session".
  await expect(card).not.toHaveClass(/active/);

  await card.locator('button[title="Edit regions"]').click();
  ...
  // Still not in session — opening the resource for region editing didn't start a timer.
  await expect(card).not.toHaveClass(/active/);
  ...
  // Closing the player returns to the resources list, not a session-log form.
  await page.locator('button[title="Close player"]').click();
  await expect(page.locator(".modal-resource-link--local", { hasText: "Practice Track" })).toBeVisible();
  await expect(page.locator("textarea")).toHaveCount(0); // no session notes/log form present
