...
const mockCatalogSongs = {
  songs: [
    ...
    { id: 2, name: "Second Song", artist_id: 2, artist_name: "OtherArtist",
      tuning_id: 1, tuning_name: "Standard", bpm: 120, active: true,
      resources: null, tags: [], seconds: null, session_type: "song",
      created_timestamp: 0, updated_timestamp: 0, meta: {} },
  ],
...
const fileB = {
  path: "/Songs/OtherArtist-Second Song-02-02-2024.gp",
  filename: "OtherArtist-Second Song-02-02-2024.gp",
  modified_ms: 1706832000000,
  size_bytes: 22222,
};

// ─── Setup ────────────────────────────────────────────────────────────────────
//
// These tests pin down a fast path: when the raw directory listing (path +
// mtime + size for every file) is byte-for-byte identical to the last scan,
// the whole pipeline (catalog fetch, dedup, analysis) should be skipped
// entirely — not just the per-file analysis step. If the listing changed at
// all, the full pipeline must still run (needed for correct undated-alias
// resolution, matching, etc).
...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript((initialEntries) => {
    ...
    (window as unknown as { __scanEntries: unknown[] }).__scanEntries = initialEntries;
    ...
    window.__TAURI_INTERNALS__ = {
      invoke: function(cmd: string, args?: Record<string, unknown>) {
        ...
        if (cmd === "scan_gp_directory") {
          return Promise.resolve(JSON.stringify((window as unknown as { __scanEntries: unknown[] }).__scanEntries));
        }
  ...
  await page.route("**/127.0.0.1:8080/api/v2/song**", (route) => {
    catalogFetchCount++;
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(mockCatalogSongs) });
  });
...
async function analyzeCallCount(page: Page) {
  return page.evaluate(() => (window as unknown as { __analyzeCallCount: () => number }).__analyzeCallCount());
}

async function setScanEntries(page: Page, entries: unknown[]) {
  await page.evaluate((entries) => {
    (window as unknown as { __scanEntries: unknown[] }).__scanEntries = entries;
  }, entries);
}
...
test("a second scan with an unchanged file listing skips the catalog fetch and analysis entirely", async ({ page }) => {
...
test("a scan after the file listing changes still runs the full pipeline", async ({ page }) => {
  await navigateToGpLibrary(page);
  await page.fill("#gp-root-path", "/Songs");
  ...
  // A new file appears — the listing is no longer identical to last time.
  await setScanEntries(page, [fileA, fileB]);
  await scanAndWait(page);
  expect(catalogFetchCount).toBe(2);
  expect(await analyzeCallCount(page)).toBe(2);
});
